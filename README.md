...
- Inline-клавиатура MAX: главное меню, список задач, добавление, редактирование и удаление напоминаний кнопками.
- Настройка часового пояса пользователя при первом запуске; timezone хранится отдельно для каждого пользователя.
- Человеческая первичная настройка: Люми спрашивает город и текущее время одним сообщением и сохраняет их для будущей погоды и напоминаний.
- Имя ассистента — Люми.
- Голосовые сообщения MAX обрабатываются через встроенное поле расшифровки `transcription`, без отдельного STT-сервиса. Для polling/webhook также нужен тип события `message_edited`, если MAX присылает расшифровку вторым событием.

## Запуск для разработки

При запуске из корня проекта бот автоматически загружает значения из `.env`. Переменные, заданные непосредственно в окружении shell, имеют приоритет.
...
Для локальной установки SearXNG в `.local/` выполните один раз:

```bash
./scripts/install-searxng.sh
```

После установки запуск на macOS/Linux:

```bash
./scripts/start-searxng.sh
```

Сервис запускается в фоне. Остановить его можно отдельной командой:
...
- `/timezone` - изменить часовой пояс пользователя.

При настройке часового пояса можно выбрать регион, указать IANA-зону вручную или сообщить текущее местное время. В последнем случае бот сохраняет вычисленное фиксированное смещение вида `UTC+03:00`; для корректного учета сезонных переходов предпочтительнее выбрать региональную зону, например `Europe/Moscow`.

## Клавиатура и напоминания

После `/start` или события `bot_started` бот показывает главное меню. Раздел «Напоминания» выводит активные задачи с отдельными кнопками «Изменить» и «Удалить». Новое напоминание создается одним сообщением в свободной форме, например `через 30 минут позвонить маме`; редактирование работает так же. Для callback-кнопок в polling необходимо получать тип обновления `message_callback` вместе с `message_created`.
